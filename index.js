...
async function startBot(sessionId = 'killua~default') {
  ...
  if (!conn.authState.creds.registered) {
    ...
    const phoneNumber = await question(chalk.cyan('Enter your WhatsApp number (e.g., 243906905464): '));
...
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'sessiongen/index.html'));
});
...
app.get('/pairing', async (req, res) => {
  ...
  try {
    if (!currentSock?.authState?.creds?.registered) {
      ...
      return res.json({ error: "Déjà connecté" });
    }
  } catch (e) {
...
app.listen(PORT, () => console.log("Web actif sur port " + PORT));
